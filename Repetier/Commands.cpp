...
void Commands::processGCode(GCode *com) {
  ...
  switch (com->G) {
    ...
    case 229: {
        ...
        const float x = radius * sin(60 * M_PI / 180.0);
        const float y = radius * 0.5;
        const float position[8][2] = {{0, 0}, { -x, -y}, {0, -radius}, {x, -y}, {x, y}, {0, radius}, { -x, y}, {0, 0}};
        float distance[8];
        Printer::moveTo(position[0][0], position[0][1], IGNORE_COORDINATE, IGNORE_COORDINATE, EEPROM::zProbeXYSpeed());
        distance[0] = Printer::runZProbe(true, false, Z_PROBE_REPETITIONS, false);
        for (uint8_t i = 1; i < 7; i++)
        {
          Printer::moveTo(position[i][0], position[i][1], IGNORE_COORDINATE, IGNORE_COORDINATE, EEPROM::zProbeXYSpeed());
          distance[i] = Printer::runZProbe(false, false);
        }
        Printer::moveTo(position[7][0], position[7][1], IGNORE_COORDINATE, IGNORE_COORDINATE, EEPROM::zProbeXYSpeed());
        distance[7] = Printer::runZProbe(false, true);
        Printer::updateCurrentPosition();
        distance[0] = Printer::zLength - Printer::currentPosition[Z_AXIS];
        Com::printArrayFLN(Com::tZProbeDistance, distance, 8, 2);
